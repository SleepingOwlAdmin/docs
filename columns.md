...
Для получения списка названия ролей нам необходимо выполнить что-то `$user->roles->pluck('title')`, что равноценно для получения данных в колонке (`roles.title`), т.е. когда класс попытается получить значение по данному ключу, он разобъет данный ключ на массив по знаку `.` и сделает следующее:
...
Для получения названия департамента нам необходимо выполнить что-то `$user->department->title`, что равноценно для получения данных в колонке (`department.title`), т.е. когда класс попытается получить значение по данному ключу, он разобъет данный ключ на массив по знаку `.` и сделает следующее:
